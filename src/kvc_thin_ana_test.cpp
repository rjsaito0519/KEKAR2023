...
static const TString pdf_name =  OUTPUT_DIR + "/img/kvc_1cm_pos_scan_analysis_test.pdf";

std::unordered_map<std::string, std::vector<FitResult>> analyze(Int_t run_num, TFile *fout, Int_t start_or_end = 0) // 0: mid_page, 1:start_page, 2: end_page, 3: both
{   
    // +---------+
    // | setting |
    // +---------+
    ...
    for (Int_t ch = 0; ch < conf.max_kvc_ch; ch++) {
        ...
        if (ch == 1) {
            ...
            coeff_a = 2.0/(conf.kvc_thin_opg[58][ch].first + conf.kvc_thin_opg[58][ch+conf.max_kvc_ch].first);
            coeff_b = 0.0;

            // coeff_a = (1.0/conf.kvc_thin_opg[58][ch].first + 1.0/conf.kvc_thin_opg[58][ch+conf.max_kvc_ch].first)/2.0;
            // coeff_b = 0.0;
    ...
    for (Int_t ch = 0; ch < conf.max_kvc_ch; ch++) {
        ...
        if (std::binary_search(should_hit_ch.begin(), should_hit_ch.end(), ch)) cutoff_threshold = 20.0;
...
Int_t main(int argc, char** argv) {
    ...
    // +--------------------------+
    TString output_path = OUTPUT_DIR + "/root/kvc_thin_pos_scan_analysis_test.root";
    ...
    for (Int_t i = 0, n_run_num = ana_run_num.size(); i < n_run_num; i++) {
        ...
        std::unordered_map<std::string, std::vector<FitResult>> result_container = analyze(ana_run_num[i], fout, pdf_save_mode);
