...
{   
    ...
    // | load root file |
    // +----------------+
    auto *f = new TFile( Form( "../root/run%05d_Elph.root", run_num ) );
    TTreeReader reader("tree", f);
    TTreeReaderArray<Double_t> baca(reader, "E72BACa");

    // +-------------------+
    // | Prepare histogram |
    // +-------------------+
    auto *h = new TH1D(Form("BACa_%d", ch+1), Form("run%05d BAC(ADC) ch%d", run_num, ch+1), adc_bin_num, adc_min, adc_max);
    ...
    std::vector<Double_t> par = param::bac_opg.at(Form("%05d-%d", run_num, ch));
    Double_t n_gauss = par[0];
    Double_t fit_range_left  = par[1];
    Double_t fit_range_right = par[2];
    
    h->GetXaxis()->SetRangeUser(fit_range_left-5, fit_range_right+5);
    ...
    for (Int_t i = 1; i < n_gauss; i++) func_str += Form(" + [%d]*TMath::Gaus(x, [1]+%d*[3], TMath::Sqrt(%d)*[4], true)", i+4, i, i);
    ...
    f_fit->SetParameter(3, (par[2]-par[1]-2*result[2])/par[0] * 0.9);
    f_fit->SetParameter(4, result[2]);
    for (Int_t i = 1; i < n_gauss; i++) {
        f_fit->SetParameter(i+4, result[0]/(2*i));
        f_fit->SetParLimits(i+4, 0, 0x1000000);
    }
    f_fit->SetLineColor(kOrange);
    f_fit->SetLineWidth(2);
    h->Fit(f_fit, "0", "", fit_range_left, fit_range_right);
    result.clear();
    for (Int_t i = 0; i < 8; i++) result.push_back(i < f_fit->GetNpar() ? f_fit->GetParameter(i) : TMath::QuietNaN());
    for (Int_t i = 0; i < 8; i++) result.push_back(i < f_fit->GetNpar() ? f_fit->GetParError(i)  : TMath::QuietNaN());
    ...
    for (Int_t i = 1; i < n_gauss; i++) {
        ...
        f_single_gaus->SetParameters(result[i+4], result[1]+i*result[3], TMath::Sqrt(i)*result[4]);
...
Int_t main(int argc, char** argv) {
    ...
    c->Divide(2, 2);
    for (Int_t ch = 0; ch < max_bac_ch; ch++) analyze(std::atoi(argv[1]), ch, c, ch+1);
