...
std::unordered_map<std::string, std::vector<FitResult>> analyze(Int_t run_num_beam, Int_t run_num_ped, Int_t hv, TFile *fout, Int_t start_or_end = 0) // 0: mid_page, 1:start_page, 2: end_page, 3: both
{   
    // +---------+
    // | setting |
    // +---------+
...
Int_t main(int argc, char** argv) {
    ...
    // +--------------------------+
    TString output_path = OUTPUT_DIR + "/root/explain_bac_hv_threshold_scan.root";
    ...
    for (Int_t i = 0, n_run_info = run_info.size(); i < n_run_info; i++) {
        ...
        std::unordered_map<std::string, std::vector<FitResult>> result_container = analyze(run_info[i][0], run_info[i][1], run_info[i][2], fout, pdf_save_mode);
