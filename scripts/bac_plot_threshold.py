...
if __name__ == '__main__':
    ...
    for hv in [56, 57, 58]:
        ...
        for threshold in threshold_list:
            indices = np.where((run_min < tree["run_num"]) * (tree["run_num"] <= run_max) * (tree["hv"] == hv) * (tree["threshold"] == threshold))[0]
            if len(indices) != 0:
                npe_th = pos_scan_tool.pair(tree["onsum_thre_val"][indices[0]][0], tree["onsum_thre_err"][indices[0]][0])
                threshold_container.append(npe_th)
            else:
                threshold_container.append(pos_scan_tool.pair(np.nan, np.nan))

        data.append([it.val for it in threshold_container])
        err.append([it.err for it in threshold_container])
    
    fig = plt.figure(figsize=(8, 8))
    ax  = fig.add_subplot(111)
    for i in range(3):
        ax.errorbar(threshold_list, data[i], yerr = err[i], marker = "o", ls = "--")
    plt.show()